...
[[outputs.http]]
  ...
  ## If the tag does not exist on a metric, no header will be added.
  ## If both a static http header and a tag-based http header exists, the
  ## value from the tag will be used.
  ## In batch mode, metrics are grouped into smaller batches by the tags
  ## in this setting. Each smaller batch of metrics is then sent as separate
  ## http requests with thier unique combination of http headers.
  ## Note: all mapped tags are removed from the metric
